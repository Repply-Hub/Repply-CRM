...
- **`verify_jwt` do `whatsapp-webhook`**: não aparece em `supabase/config.toml` (só `automacao-diaria`, `portal-scraper`, `import-licencas`, `gmail-sync-inbox`, `gmail-callback` têm `verify_jwt = false`). Se o default for `verify_jwt = true`, a uazapi precisaria enviar um JWT válido para chamar o webhook — o que normalmente não é o caso para webhooks externos. **Vale confirmar no painel do Supabase se essa função está realmente com verificação de JWT desabilitada**, senão o webhook nunca funcionará em produção.
