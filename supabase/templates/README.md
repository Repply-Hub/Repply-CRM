...
| `resumo-diario.html` | **nenhum painel** — é montado pela função de servidor da pauta | marcadores `{{PAUTA_*}}`, ver abaixo |
...
**`{{PAUTA_ALGO}}`** (maiúsculas, sem ponto, sem espaço) é nossa, só do `resumo-diario.html`,
e é a função de servidor que substitui. O Supabase nunca vê esse arquivo.
