-- ============================================================================
-- RADAR DE RISCO: EDITAR CAMPO NAO E MOVIMENTO
-- ============================================================================
-- As duas funcoes mediam "ha quanto tempo este negocio nao anda" olhando a linha mais recente
-- de pedidos_historico_status, SEM olhar o tipo. Essa tabela guarda duas coisas diferentes na
-- mesma pilha: mudanca de etapa (tipo = status) e edicao de campo (tipo = campo). Entao corrigir
-- uma observacao fazia o negocio parecer que tinha andado, e o cronometro voltava a zero.
--
-- Medido em 22/09/2026, na base de producao:
--   - tipo = campo so comeca em 01/09/2026 e ja tem 13.179 linhas em 5.157 negocios;
--     tipo = status tem 13.047 linhas desde 27/07/2025. Em 3 semanas a edicao de campo passou
--     um ano inteiro de movimentacao real.
--   - 08/09/2026: uma operacao em massa gravou 9.498 edicoes em 4.630 negocios NUM DIA. Para o
--     radar, todos os 4.630 "andaram" naquele dia e sumiram da lista de parados por 7 dias.
--     Toda correcao em massa cega o radar por uma semana.
--   - Ensaio em transacao desfeita: 26 negocios da MD tem o "parado ha X dias" corrigido, o
--     maior deles em 14 dias, somando R$ 3,5 mi. A CONTAGEM de negocios em risco nao muda hoje
--     (102, R$ 5.084.868 antes e depois) porque a regra "sem proxima acao" ja segurava os 6 que
--     o defeito escondia -- uma regra tapando o buraco da outra, por acaso.
--
-- Decisao do dono do produto em 22/09/2026: editar campo NAO e cuidar do negocio.
--
-- O filtro copia o que pauta_do_dia_de ja fazia certo desde sempre (h.tipo = status). O indice
-- idx_pedidos_historico_status_pedido_created (pedido_id, created_at DESC) continua servindo:
-- a condicao so pula as linhas de edicao, e cada negocio tem ~2 linhas de historico.
--
-- Mesma assinatura -> CREATE OR REPLACE preserva as concessoes (dashboard aberta a
-- authenticated; _de fechada). Sem DROP. Unica mudanca em relacao a 20260917120000: a linha
-- "AND h.tipo = 'status'" dentro das duas LATERAL.
-- Rollback: reaplicar 20260917120000_pede_atencao_sem_proxima_acao.sql.
-- ============================================================================
