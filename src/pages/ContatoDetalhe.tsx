...
function obrasDoEmbed(contato: unknown): string[] | null {
  const vinculos = (
    contato as { vinculos_obra?: { obra?: { id?: string } }[] } | undefined
  )?.vinculos_obra;
...
const ContatoDetalhe = () => {
  ...
  const { data: tarefasKanbanColunas = [] } =
    useTarefasKanbanColunas(empresaId);
  ...
  const totalTarefasPages = Math.max(
    1,
    Math.ceil(tarefasRelacionadas.length / TAREFAS_PAGE_SIZE),
  );
  const paginatedTarefas = useMemo(
    () =>
      tarefasRelacionadas.slice(
        (tarefasPage - 1) * TAREFAS_PAGE_SIZE,
        tarefasPage * TAREFAS_PAGE_SIZE,
      ),
  ...
  const [editData, setEditData] = useState({
    ...
    cargo: "",
    empresa: "",
    // Id do cliente escolhido no dropdown de Empresa. O texto `empresa` continua
    // guardado em paralelo para o caso de contato antigo vinculado só por nome.
    clienteId: "",
  ...
  const openEdit = () => {
    ...
    setEditData({
      ...
      // Preenche o dropdown com a empresa que o contato já tem: pela chave
      // (`cliente_id`) quando existe, ou casando o nome em texto com a carteira de
      // clientes para os contatos antigos que só têm o vínculo por nome.
      clienteId: contato.cliente_id ?? clienteVinculado?.id ?? "",
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) return;
    try {
      // Empresa vem do dropdown: com um cliente escolhido, grava também a CHAVE
      // (`cliente_id`), não só o nome em texto. Vincular só pelo texto era o que
      // deixava o contato invisível em toda consulta que filtra por `cliente_id`.
      const empSelecionada = editData.clienteId
        ? clientes?.find((c) => c.id === editData.clienteId)
        : undefined;
      await updateContato.mutateAsync({
        ...
        cargo: editData.cargo || null,
        empresa: empSelecionada
          ? empSelecionada.empresa
          : editData.empresa || undefined,
        ...(empSelecionada ? { cliente_id: empSelecionada.id } : {}),
      });
      ...
      if (editData.obraIds !== null) {
        try {
          await salvarObrasDoContato.mutateAsync({
            contatoId: id,
            obraIds: editData.obraIds,
          });
        } catch {
          toast.warning(
            "Contato salvo, mas não deu para atualizar as obras vinculadas.",
          );
        }
      }
  ...
  return (
    ...
    >
      <div className="p-3 sm:p-4 md:p-6 space-y-6 w-full">
        {/* Editar saiu do cabeçalho e ficou aqui, no corpo da página. Ganhou um título
            ao lado para não ficar solto num canto. */}
        ...
        {/* items-start: sem isso a grade estica o cartão mais baixo para a altura do
            mais alto, e "Canais de Comunicação" ficava com um vão vazio embaixo. */}
        <div className="grid gap-6 md:grid-cols-3 items-start">
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="space-y-4">
              <div className="p-4 rounded-xl bg-primary/5 border border-primary/10">
                ...
                {clienteVinculado ? (
                  <Button
                    ...
                    onClick={() => {
                      const slug = slugify(
                        clienteVinculado.empresa || "cliente",
                      );
              ...
              <div className="rounded-xl border divide-y">
                <div className="flex items-center justify-between text-sm px-4 py-2.5">
                ...
                <div className="flex items-center justify-between text-sm px-4 py-2.5">
                  ...
                  <span className="font-medium">
                    {contato.created_at
                      ? format(new Date(contato.created_at), "dd/MM/yyyy", {
                          locale: ptBR,
                        })
                      : "—"}
                  </span>
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              {/* Canais empilhados em linhas de largura cheia (antes eram duas colunas
                  curtas). Ficam maiores, os botões viram alvo de clique de verdade, e o
                  cartão não sobra espaço. */}
              <div className="rounded-xl border bg-muted/20 divide-y">
                <div className="flex items-start gap-4 p-4">
                  <div className="h-11 w-11 rounded-lg bg-background flex items-center justify-center border shadow-sm shrink-0">
                    ...
                    {contato.email && (
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        <Button
                          ...
                          onClick={() => copyInfo("E-mail", contato.email)}
                        >
                    ...
                    {contato.telefone && (
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          className="h-7 gap-1.5 text-xs text-muted-foreground"
                          onClick={() => copyInfo("Telefone", contato.telefone)}
                        >
        ...
        {temTarefas === true && (
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <Card className="md:col-span-3">
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <ListChecks className="h-4 w-4" /> Tarefas Vinculadas (Via
                  Empresa)
                </CardTitle>
                <div className="flex items-center gap-2">
                  <Badge variant="outline">
                    {tarefasRelacionadas.length} Registros
                  </Badge>
                  {clienteVinculado && (
                    <Button
                      size="sm"
                      onClick={() => setAddTarefaOpen(true)}
                      className="gap-1.5"
                    >
              ...
              <CardContent>
                {!clienteVinculado ? (
                  <div className="py-10 text-center border-2 border-dashed rounded-xl">
                    <p className="text-sm text-muted-foreground">
                      Vincule este contato a uma empresa para ver e criar
                      tarefas.
                    </p>
                  </div>
                ) : loadingTarefas ? (
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  {contato.nome_contato}" será removido permanentemente.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditData((d) => ({ ...d, nome_contato: e.target.value }))
                  }
                  required
                />
              </div>
              ...
              <Dialog open={vincularOpen} onOpenChange={setVincularOpen}>
                <ConteudoDialogo className="sm:max-w-[425px]">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      onClick={async () => {
                        const emp = clientes?.find(
                          (c) => c.id === selectedEmpresaId,
                        );
                        if (emp && id) {
                          try {
                            await updateContato.mutateAsync({
              ...
              {temObras !== false &&
                clienteVinculado?.obras &&
                clienteVinculado.obras.length > 0 && (
                  <div>
                    ...
                    <div className="mt-1 max-h-40 space-y-1 overflow-y-auto rounded-md border border-border p-2">
                      {editData.obraIds === null && (
                        <p className="px-1.5 py-1 text-xs text-muted-foreground">
                          Não foi possível carregar os vínculos. Feche e abra a
                          edição de novo — nada será alterado nas obras deste
                          contato.
                        </p>
                      )}
                      {editData.obraIds !== null &&
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={updateContato.isPending}
              >
      ...
      <ConfirmarEnviarEmailDialog
        ...
        onConfirmar={(endereco) =>
          navigate(`/emails?to=${encodeURIComponent(endereco)}`)
        }
      />
    </AppLayout>
  );
