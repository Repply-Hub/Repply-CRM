...
export function CampoDeAnexosDaTarefa({
  ...
  // Hook compartilhado com o campo de anexo do negócio: processa a seleção vinda do input
  // (`multiple`) OU do arrastar-e-soltar, valida cada arquivo com `recusaDoAnexoDeTarefa` e
  // entrega os aceitos de uma vez. `desabilitado` cobre tanto o input quanto o arrasto — sem
  // isso, quem tem `somenteLeitura`/sem `onRemover` continuaria podendo soltar arquivos por cima
  // do campo.
  const selecao = useSelecaoDeArquivos({
    recusar: recusaDoAnexoDeTarefa,
    onArquivos: (aceitos) => {
