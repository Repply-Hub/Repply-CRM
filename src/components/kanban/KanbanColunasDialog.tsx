...
export function KanbanColunasDialog({ open, onOpenChange }: Props) {
    ...
    const totalItems = colunas?.length ?? 0;
    const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
    const paginatedColunas = (colunas ?? []).slice((page - 1) * pageSize, page * pageSize);
                        ...
                        <ScrollArea className="max-h-[400px] -mx-1 px-1">
                            {isLoading ? (
                                ...
                                <div className="space-y-1.5">
                                    {paginatedColunas.map((c) => {
                                        const idxGlobal = colunas!.findIndex(x => x.id === c.id);
                                            ...
                                            <div key={c.id} className="flex items-center gap-2 rounded-md border bg-card p-2">
                                                <div className="flex flex-col">
                                                    <button
                                                        type="button"
                                                        onClick={() => move(c.id, -1)}
                                                        disabled={idxGlobal === 0 || reorderMut.isPending}
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => move(c.id, 1)}
                                                        disabled={idxGlobal === (colunas!.length - 1) || reorderMut.isPending}
                                                        className="text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed"
