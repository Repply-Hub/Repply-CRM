/**
 * Reconhece publicações de licença ambiental — Licença Prévia (LP), de Instalação (LI) e
 * de Operação (LO) — dentro do texto cru de uma edição do Diário Oficial de Extremoz/RN.
 *
 * É lógica pura, sem I/O, para poder ser testada (`dom-extremoz-licencas.test.ts`) e
 * reaproveitada. A Edge Function `scrape-extremoz-licencas` roda em Deno e NÃO consegue
 * importar deste arquivo — ela carrega uma cópia em
 * `supabase/functions/scrape-extremoz-licencas/licencas.ts`. **As duas precisam
 * concordar**; é a mesma situação de `normalizeWhatsappPhone` (CLAUDE.md §7.1) e da lib
 * gêmea do DOM Natal (`dom-natal-licencas.ts`). Se mexer aqui, mexa lá.
 *
 * ── De onde vieram as heurísticas ──────────────────────────────────────────────────
 * As regexes de tipo de licença, fase de obra, descrição de obra, e-mail e razão social
 * são portadas de `supabase/functions/portal-scraper/index.ts` (o scraper antigo de
 * Extremoz, que rodava no navegador). O filtro anti-portaria (`pareceContextoNormativo`)
 * e o padrão de CNPJ vêm da lib do DOM Natal. O que este arquivo acrescenta é: dedupe por
 * hash de bloco, estrutura tipada e testes.
 *
 * ── O campo `prioridade` ───────────────────────────────────────────────────────────
 * 🔴 `licencas_extremoz.prioridade` NÃO é prioridade — é a FASE DA OBRA (implantação,
 * instalação, operação...). O nome da coluna mente, igual a `prazo_resposta` em `pedidos`
 * (CLAUDE.md §4.4). A tela já rotula "Fase da Obra". Não renomear a coluna, não usar como
 * prioridade de coisa nenhuma.
 */
