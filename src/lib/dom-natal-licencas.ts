 ...
 * de Operação (LO) — dentro do texto cru de uma edição do Diário Oficial do Município de
 * Natal (DOM).
 *
 ...
 * nunca traz LP/LI/LO — a investigação da Fase 1 varreu 10 edições reais e achou zero
 * publicações verdadeiras, só um FALSO POSITIVO: a frase "emissão de Licença de Operação"
 * dentro de uma portaria normativa (art. 6º de um regulamento da SEMURB). Capturar isso
 * enche a tela de prospecção de lixo. Então a regra aqui exige, além da menção ao tipo de
 * licença, um SINAL de que é mesmo uma publicação de ato concreto (requerimento/concessão,
 * número de licença, CNPJ do empreendedor) e REJEITA o que cheira a texto de norma.
 */
